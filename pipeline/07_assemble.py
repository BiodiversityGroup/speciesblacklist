...
Two lists come out of this pipeline, and they make different claims:
  ...
  LIST 1 -- Data Deficient.  IUCN has assessed these and returned "we don't know".
            Ranked by restriction tier; tranche A (tier 3-5) is the validated
...
Outputs: build_2026/dd_candidates.csv   (unverified; 09 produces the published list)
         build_2026/SPECIES_BLACK_LIST_ne.csv
"""
import csv, collections, importlib.util, os, re, sys

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from _common import class_for, normalise_class, sentences, best_evidence
...
def main():
    ...
    with open(os.path.join(BUILD, "dd_candidates.csv"), "w", newline="",
              encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=list(rows[0])); w.writeheader(); w.writerows(rows)

    ne = list(csv.DictReader(open(os.path.join(BUILD, "black_list_not_evaluated.csv"),
                                  encoding="utf-8")))
    # Collect every candidate then apply the shared selector, rather than deciding
    # pairwise as sentences stream past. The streaming version could not see a better
    # sentence that arrived after a merely-acceptable one had already been kept.
    ne_names = {r["book_name"] for r in ne}
    ne_hits = collections.defaultdict(list)
    for site, text in ex.blocks():
        for sent in sentences(text):
            for g, e in ex.BINOMIAL.findall(sent):
                n = f"{g} {e}"
                if n in ne_names:
                    ne_hits[n].append((site, sent, text))
    ne_found = {n: (h[0], h[1]) for n, h in
                ((n, best_evidence(hs, ex.score)) for n, hs in ne_hits.items())}
    ...
    with open(os.path.join(BUILD, "ne_candidates.csv"), "w", newline="",
              encoding="utf-8") as fh:
    ...
    print(f"  tranche A (tier 3-5, validated): {a:,}")
    print(f"  tranche B (tier 1-2)           : {len(rows)-a:,}")
